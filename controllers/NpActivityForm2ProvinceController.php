...
class NpActivityForm2ProvinceController extends AppController
{
    public $title = 'บันทึกผลการปฏิบัติงานประจำเดือน (จพล. ประจำจังหวัด)';
    public $subTitle = 'การรายงานผลการปฏิบัติงานประจำเดือน';
	public $enableCsrfValidation = false;
    public $imagePath = '/npactivityform2province';
    public $plaseSelect = array('' => 'กรุณาเลือก');
    ...
    public function actionIndex()
    {
    	...
    	$model = new NP_ACTIVITY_FORM2_PROVINCE();
    	
        $rtparams = $request->post('rtparams'); $rtparams = (empty($rtparams)?$request->get('rtparams'):$rtparams);
        $workId = $request->post('work_id'); $workId = (empty($workId)?$request->get('work_id'):$workId);
        $arrAct =array();
        ...
        $this->subTitle = $workId=='6'? 'การบูรณาการภายในจังหวัด': 'การบูรณาการภายนอกจังหวัด';

    	$params = array('title' => $this->title.' > '.$this->subTitle,
                        'currentYear' => $currentYear, 
                        'currentMonth' => $currentMonth,
    					'rtparams' => $rtparams,
    					'arrPlaseSelect' => $this->plaseSelect,
    					'arrAct' =>$arrAct,
    					'arrAct2' =>$arrAct2);
        ...
        //return $this->render('index', array('$NP_ACTIVITY_FORM2_PROVINCE_id' => $NP_ACTIVITY_FORM2_PROVINCE_id, 'sbMainProj' => $sbMainProj));
    }
...
//     	$this->title = 'NP001';
    	...
    	$backAction = ['npactivityform2province/', 'rtparams'=>$rtparams];
    	
    	$backAction = BaseArrayHelper::merge($backAction, $tmp_params);
    	
    	$this->subTitle = $work_id=='6'? 'การบูรณาการภายในจังหวัด': 'การบูรณาการภายนอกจังหวัด';
    	
    	$this->addBreadCrumb('NP003 '.$this->title, Url::to(['npworkprovince/']));
    	$this->addBreadCrumb($this->subTitle, Url::to($backAction));
    	$this->addBreadCrumb('บันทึก '.$this->subTitle);
...
// 			$backAction = ['npactivityform1amphoe/'];
	    	...
	    	$input_npactivity = $request->post('NP_ACTIVITY_FORM2_PROVINCE');
            $npactivity;
	    	if($mode=='edit'){
	    		$id = $input_npactivity['NP_ACTIVITY_FORM2_PROVINCE_ID'];
	    		$npactivity = NP_ACTIVITY_FORM2_PROVINCE::findOne($id);
	    	}else if($mode=='add'){
	    		$npactivity = new NP_ACTIVITY_FORM2_PROVINCE();
	    		$year = $input_npactivity['YEAR'];
	    		$npactivity->NP_ACTIVITY_FORM2_PROVINCE_ID = NP_ACTIVITY_FORM2_PROVINCE::getNewID();
	    		$npactivity->IMAGE_PATH = Utils::uploadFile(['image_path' => $this->imagePath, 
                                                            'file_name' => 'IMAGE_PATH', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_PROVINCE_ID,
                											'year' => $year]);
                $npactivity->IMAGE_PATH_1 = Utils::uploadFile(['image_path' => $this->imagePath, 
                                                            'file_name' => 'IMAGE_PATH_1', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_PROVINCE_ID.'-1',
                											'year' => $year]);
                $npactivity->IMAGE_PATH_2 = Utils::uploadFile(['image_path' => $this->imagePath, 
                                                            'file_name' => 'IMAGE_PATH_2', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_PROVINCE_ID.'-2',
                											'year' => $year]);
                $npactivity->IMAGE_PATH_3 = Utils::uploadFile(['image_path' => $this->imagePath, 
                                                            'file_name' => 'IMAGE_PATH_3', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_PROVINCE_ID.'-3',
                											'year' => $year]);
                $npactivity->IMAGE_PATH_4 = Utils::uploadFile(['image_path' => $this->imagePath, 
                                                            'file_name' => 'IMAGE_PATH_4', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_PROVINCE_ID.'-4',
                											'year' => $year]);
	    		$npactivity->STATUS = 'A';
	    	...
	    	foreach ($input_npactivity as $key=>$value) {
	    		switch (strtoupper($key)) {
	    			case 'NP_ACTIVITY_FORM2_PROVINCE_ID':
	    				continue 2;
	    				break;
	    			case 'START_DATE':
    		...
    		$backAction[0] = 'npactivityform2province/add';
    		$backAction['id'] = $id;
 	...
 	public function actionGridview()
    {
		...
		$np_activity_form2_province_id = $request->post('np_activity_form2_province_id'); $np_activity_form2_province_id = (empty($np_activity_form2_province_id)? $request->get('np_activity_form2_province_id'): $np_activity_form2_province_id);
    	...
    	switch($oper){
    		case 'request' :
    				...
    				foreach($filters->rules as $conditions){
    				    switch($conditions->field){
    				        ...
    				        default:
                                ...
                                if ($conditions->field != 'NP_ACTIVITY_FORM2_PROVINCE.NP_WORK_GROUP_ACTIVITY_ID') {
                ...
                $count = NP_ACTIVITY_FORM2_PROVINCE::find()->where($where_causes)->count();
                //$result = NP_ACTIVITY_FORM2_PROVINCE::find()->joinWith('WA_PROVINCE')->where($where_causes)->orderBy($sidx.' '.$sord)->offset($offset)->limit($rows)->asArray()->all();

                //$command = Yii::$app->db->createCommand("select NP_ACTIVITY_FORM2_PROVINCE.* ,from NP_ACTIVITY_FORM2_PROVINCE a join WA_PROVINCE b on (a.PROVINCE_CODE = b.PROVINCE_CODE) join WA_AMPHOE c on (C.AMPHOE_CODE = a.AMPHOE_CODE)");
                $query = new Query;
                $query->select('NP_ACTIVITY_FORM2_PROVINCE.*,
                                NP_SP_OPEN_PROVINCE.STATUS AS STATUS_2,
                                WA_PROVINCE.PROVINCE_NAME_TH')
	    			->from('NP_ACTIVITY_FORM2_PROVINCE')
                    ->leftJoin('NP_SP_OPEN_PROVINCE', 'NP_ACTIVITY_FORM2_PROVINCE.YEAR=NP_SP_OPEN_PROVINCE.YEAR
                                                        AND NP_ACTIVITY_FORM2_PROVINCE.MONTH=NP_SP_OPEN_PROVINCE.MONTH
                                                        AND NP_ACTIVITY_FORM2_PROVINCE.PROVINCE_CODE=NP_SP_OPEN_PROVINCE.PROVINCE_CODE')
	    			->innerJoin('WA_PROVINCE', 'NP_ACTIVITY_FORM2_PROVINCE.PROVINCE_CODE=WA_PROVINCE.PROVINCE_CODE')
	    			->where($where_causes)
	    			->orderBy($sidx.' '.$sord)
	    			->offset($offset)
	    			->limit($rows);
				...
				for ($i=0; $i<count($result); $i++) {
					...
					array_push($response->rows, array('id'=>$result[$i]['NP_ACTIVITY_FORM2_PROVINCE_ID'], 
													'cell'=>array($seq,
																	$result[$i]['SUBJECT'],
																	$result[$i]['START_DATE'],
																	$result[$i]['YEAR'],
			...
			case 'del':
                try {
                    ...
                    NP_ACTIVITY_FORM2_PROVINCE::deleteAll(['in', 'NP_ACTIVITY_FORM2_PROVINCE_ID', $arrId]);
