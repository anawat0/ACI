...
class SsbMainProjController extends AppController
{
    ...
    public function actionIndex()
    {
        ...
        $this->addBreadCrumb('SSB001 รายชื่อโครงการ สวฬ.');
    ...
    public function actionGridview()
    {
        ...
        switch($oper)
        {
            case 'request':
                ...
                $count = SSB_MAIN_PROJ::find()->where($where_causes)->count();
                $result = SSB_MAIN_PROJ::find()->where($where_causes)->orderBy($sidx.' '.$sord)->offset($offset)->limit($rows)->asArray()->all();
                ...
                for($i=0; $i<$countResult; $i++) {
                    ...
                    array_push($response->rows, array('id' => $result[$i]['SSB_MAIN_PROJ_ID'], 
                                                      'cell' => array($seq,
            ...
            case 'edit':
                ...
                $ssbMainProj = SSB_MAIN_PROJ::findOne($id);
                $ssbMainProj->MAIN_PROJ_NAME_TH = $mainProjNameTh;
                $ssbMainProj->MAIN_PROJ_NAME_EN = $mainProjNameEn;
                $ssbMainProj->BUDGET_YEAR = $budgetYear;
                $ssbMainProj->STATUS = Utils::getStatus($status);
                $ssbMainProj->LAST_UPD_USER_ID = $user_id;
                $ssbMainProj->LAST_UPD_TIME = new \yii\db\Expression('SYSDATE');
                
                $response = new jqGridResponse();
                if ($ssbMainProj->save()) {  
                    $response->success(['id'=>$ssbMainProj->SSB_MAIN_PROJ_ID]);
                } else {
                    $response->error($ssbMainProj->getErrors());
                }

                $result = $response->response_encode();
                break;
            case 'add':
                $ssbMainProj = new SSB_MAIN_PROJ();
                $ssbMainProj->SSB_MAIN_PROJ_ID = SSB_MAIN_PROJ::getNewID();
                $ssbMainProj->MAIN_PROJ_NAME_TH = $mainProjNameTh;
                $ssbMainProj->MAIN_PROJ_NAME_EN = $mainProjNameEn;
                $ssbMainProj->BUDGET_YEAR = $budgetYear;
                $ssbMainProj->STATUS = Utils::getStatus($status);
                $ssbMainProj->CREATE_USER_ID = $user_id;
                $ssbMainProj->CREATE_TIME = new \yii\db\Expression('SYSDATE');
                $ssbMainProj->LAST_UPD_USER_ID = $user_id;
                $ssbMainProj->LAST_UPD_TIME = new \yii\db\Expression('SYSDATE');
            ...
            case 'del':
                try {
                    ...
                    SSB_MAIN_PROJ::deleteAll(['in', 'SSB_MAIN_PROJ_ID', $arrId]);
