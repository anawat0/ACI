...
use app\models\NP_ACTIVITY_FORM2_AMPHOE;
...
class NpActivityForm2AmphoeController extends AppController
{
	public $title = 'บันทึกผลการปฏิบัติงานประจำเดือน (จพล. ประจำอำเภอ)';
    public $subTitle = '';
	public $enableCsrfValidation = false;
    public $imagePath = '/npactivityform2amphoe';
    ...
    public function actionIndex()
    {
    	...
    	$model = new NP_ACTIVITY_FORM2_AMPHOE();
        ...
        $this->subTitle = ($work_id=='2'?'การบูรณาการภายในอำเภอ':'การบูรณาการภายนอกอำเภอ');
        ...
        //return $this->render('index', array('$np_activity_form2_amphoe_id' => $np_activity_form2_amphoe_id, 'sbMainProj' => $sbMainProj));
    }
...
//     	$this->title = 'NP001';
    	...
    	$backAction = ['npactivityform2amphoe/', 'rtparams'=>$rtparams];
    	
    	$backAction = BaseArrayHelper::merge($backAction, $tmp_params);
    	
    	$this->addBreadCrumb('NP001 บันทึกผลการปฏิบัติงานประจาเดือน (จพล. ประจำอำเภอ)', Url::to(['npworkamphoe/']));
    	$this->addBreadCrumb(($work_id=='2'?'การบูรณาการภายในอำเภอ':'การบูรณาการภายนอกอำเภอ'), Url::to($backAction));
    	$this->addBreadCrumb('บันทึก การบูรณาการภายในอำเภอ');
    	...
    	if(!empty($id)){
            $params['npactivity'] = NP_ACTIVITY_FORM2_AMPHOE::findOne($id);
            $params['isCurrentMonth'] = (Utils::isCurrentMonth($params['npactivity']['YEAR'],
                                                                         $params['npactivity']['MONTH'],
                                                                         true))? 'true': 'false';
    	}else{
    		$params['npactivity'] = new NP_ACTIVITY_FORM2_AMPHOE();
    	}
...
// 	    	echo base64_decode($request->post('backAction'));
// 	    	exit();
// 	    	$backAction = json_decode(base64_decode($request->post('backAction')));
			$hidden_return = $request->post('return');
// 			print_r($hidden_return);
// 			exit();

// 			$backAction = ['npactivityform1amphoe/'];
			foreach($hidden_return as $key=>$value){
				$backAction[$key]=$value;
			}
			
	    	$input_npactivity = $request->post('NP_ACTIVITY_FORM2_AMPHOE');
            $npactivity;
	    	if($mode=='edit'){
	    		$id = $input_npactivity['NP_ACTIVITY_FORM2_AMPHOE_ID'];
	    		$npactivity = NP_ACTIVITY_FORM2_AMPHOE::findOne($id);
	    	}else if($mode=='add'){
	    		$npactivity = new NP_ACTIVITY_FORM2_AMPHOE();
	    		$year = $input_npactivity['YEAR'];
	    		$npactivity->NP_ACTIVITY_FORM2_AMPHOE_ID = NP_ACTIVITY_FORM2_AMPHOE::getNewID();
	    		$npactivity->IMAGE_PATH = Utils::uploadFile(['image_path' => $this->imagePath,
                                                            'file_name' => 'IMAGE_PATH', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_AMPHOE_ID,
                											'year' => $year]);
                $npactivity->IMAGE_PATH_1 = Utils::uploadFile(['image_path' => $this->imagePath,
                                                            'file_name' => 'IMAGE_PATH_1', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_AMPHOE_ID.'-1',
                											'year' => $year]);
                $npactivity->IMAGE_PATH_2 = Utils::uploadFile(['image_path' => $this->imagePath,
                                                            'file_name' => 'IMAGE_PATH_2', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_AMPHOE_ID.'-2',
                											'year' => $year]);
                $npactivity->IMAGE_PATH_3 = Utils::uploadFile(['image_path' => $this->imagePath,
                                                            'file_name' => 'IMAGE_PATH_3', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_AMPHOE_ID.'-3',
                											'year' => $year]);
                $npactivity->IMAGE_PATH_4 = Utils::uploadFile(['image_path' => $this->imagePath,
                                                            'file_name' => 'IMAGE_PATH_4', 
                                                            'id' => $npactivity->NP_ACTIVITY_FORM2_AMPHOE_ID.'-4',
                											'year' => $year]);
	    		$npactivity->STATUS = 'A';
	    	...
	    	foreach ($input_npactivity as $key=>$value) {
	    		switch (strtoupper($key)) {
	    			case 'NP_ACTIVITY_FORM2_AMPHOE_ID':
	    				continue 2;
	    				break;
	    			case 'START_DATE':
    		...
    		$backAction[0] = 'npactivityform2amphoe/add';
    		$backAction['id'] = $id;
    		$backAction['message'] = base64_encode('บันทึกไม่สำเร็จ กรุณาติดต่อผู้ดูแลระบบ');
 	...
 	public function actionGridview()
    {
		...
		$subjectlocation = $request->post('subjectlocation');

		//Parameter from another pages.
		$np_activity_form2_amphoe_id = $request->post('np_activity_form2_amphoe_id'); $np_activity_form2_amphoe_id = (empty($np_activity_form2_amphoe_id)? $request->get('np_activity_form2_amphoe_id'): $np_activity_form2_amphoe_id);
    	...
    	switch($oper){
    		case 'request' :
    				...
    				foreach($filters->rules as $conditions){
    				    switch($conditions->field){
    				        ...
    				        default:
                                ...
                                if ($conditions->field != 'NP_ACTIVITY_FORM2_AMPHOE.NP_WORK_GROUP_ACTIVITY_ID') {
    			...
    			$count = NP_ACTIVITY_FORM2_AMPHOE::find()->where($where_causes)->count();
    			//$result = NP_ACTIVITY_FORM2_AMPHOE::find()->joinWith('WA_PROVINCE')->where($where_causes)->orderBy($sidx.' '.$sord)->offset($offset)->limit($rows)->asArray()->all();

    			//$command = Yii::$app->db->createCommand("select NP_ACTIVITY_FORM2_AMPHOE.* ,from NP_ACTIVITY_FORM2_AMPHOE a join WA_PROVINCE b on (a.PROVINCE_CODE = b.PROVINCE_CODE) join WA_AMPHOE c on (C.AMPHOE_CODE = a.AMPHOE_CODE)");
    			$query = new Query;
    			$query->select('NP_ACTIVITY_FORM2_AMPHOE.*,
                                NP_SP_OPEN_AMPHOE.STATUS AS STATUS_2,
                                WA_PROVINCE.PROVINCE_NAME_TH,
                                WA_AMPHOE.AMPHOE_NAME_TH')
	    			->from('NP_ACTIVITY_FORM2_AMPHOE')
                    ->leftJoin('NP_SP_OPEN_AMPHOE', 'NP_ACTIVITY_FORM2_AMPHOE.YEAR=NP_SP_OPEN_AMPHOE.YEAR
                                                        AND NP_ACTIVITY_FORM2_AMPHOE.MONTH=NP_SP_OPEN_AMPHOE.MONTH
                                                        AND NP_ACTIVITY_FORM2_AMPHOE.PROVINCE_CODE=NP_SP_OPEN_AMPHOE.PROVINCE_CODE
                                                        AND NP_ACTIVITY_FORM2_AMPHOE.AMPHOE_CODE=NP_SP_OPEN_AMPHOE.AMPHOE_CODE')
	    			->innerJoin('WA_PROVINCE', 'NP_ACTIVITY_FORM2_AMPHOE.PROVINCE_CODE=WA_PROVINCE.PROVINCE_CODE')
	    			->innerJoin('WA_AMPHOE', 'NP_ACTIVITY_FORM2_AMPHOE.AMPHOE_CODE=WA_AMPHOE.AMPHOE_CODE')
	    			->where($where_causes)
	    			->orderBy($sidx.' '.$sord)
	    			->offset($offset)
	    			->limit($rows);
				...
				for ($i=0; $i<$countResult; $i++) {
					...
					array_push($response->rows, array('id'=>$result[$i]['NP_ACTIVITY_FORM2_AMPHOE_ID'],
													'cell'=>array($seq,
																	$result[$i]['SUBJECT'],
																	$result[$i]['START_DATE'],
																	$result[$i]['YEAR'],
			...
			case 'del':
                try {
                    ...
                    NP_ACTIVITY_FORM2_AMPHOE::deleteAll(['in', 'NP_ACTIVITY_FORM2_AMPHOE_ID', $arrId]);
