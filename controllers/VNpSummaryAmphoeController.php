...
use app\models\V_NP_SUMMARY_AMPHOE;
...
class VNpSummaryAmphoeController extends AppController
{
    public $title = 'NP006';
    ...
    public function actionIndex()
    {
    	...
    	$params = array('currentMonth' => $currentMonth,
                        'currentYear' => $currentYear,
                        'rtparams' => $rtparams,
    					'arrPlaseSelect' => $this->arrPlaseSelect);

    	$this->addBreadCrumb('NP006 สรุปรายงานของสำนักงานส่วนภูมิภาค (จพล. ประจำอำเภอ)');
 	...
 	public function actionGridview()
    {
    	...
    	switch($oper){
    		case 'request' :
    			...
    			$count = V_NP_SUMMARY_AMPHOE::find()->where($where_causes)->count();
    			$result = V_NP_SUMMARY_AMPHOE::find()
                                    ->where($where_causes)
                                    ->orderBy($sidx.' '.$sord)
                                    ->offset($offset)
                                    ->limit($rows)
				...
				for ($i=0; $i<count($result); $i++) {
                    ...
                    if (isset($result[$i]->npSpOpenAmphoe)) {
                        $status = $result[$i]->npSpOpenAmphoe->STATUS;
    ...
    public function actionAjaxUpdateStatus()
    {
        ...
        $amphoeCode = $request->post('amphoe_code');
        $month = $request->post('month');
        $year = $request->post('year');

        if ($status == 'A') {
            $result = V_NP_SUMMARY_AMPHOE::callSpOpenRecAmphoe($amphoeCode,
                                                                $provinceCode,
                                                                $month,
                                                                $year);
            $result['status'] = 'S';
        } elseif ($status == 'S') {
            $result = V_NP_SUMMARY_AMPHOE::callSpCloseRecAmphoe($amphoeCode,
                                                                $provinceCode,
                                                                $month,
                                                                $year);
            $result['status'] = 'C';
