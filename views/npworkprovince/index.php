...
<?php
    $npwork  = NP_WORK_GROUP::find()->where("GROUP_TYPE=1 AND STATUS='A'")->all();
    ...
    <?php
        ...
        foreach ($result as $row) {  $i++ ;  
            if ($row['FORM_TYPE'] == '1') { ?>                       
                        <div><label class="col-md-8" ><a href=index.php?r=npactivityform1province&act_id=<?php echo $row['NP_WORK_GROUP_ACTIVITY_ID']; ?>&work_id=<?php echo $row['NP_W_ID']; ?>><?php echo $i." ".$GROUP_SUBJECT =$row['ACTIVITY_SUBJECT']?></a> </label></div>			
<?php       } else if ($row['FORM_TYPE'] == '2') {  ?>
                        <div><label class="col-md-8" ><a href=index.php?r=npactivityform2province&act_id=<?php echo $row['NP_WORK_GROUP_ACTIVITY_ID']; ?>&work_id=<?php echo $row['NP_W_ID']; ?>><?php echo $i." ".$GROUP_SUBJECT =$row['ACTIVITY_SUBJECT']?></a> </label></div>			
<?php       } else {?>
