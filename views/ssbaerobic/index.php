	...
	<div class="panel panel-primary">
		<div class="panel-body">
			<div class="form-group">
				...
				<div class="col-md-2">  
					<select class="form-control" onchange="Common.jqgrid.onFilter()" name="YEAR">
                        <?php Utils::getOptionsYears('', false); ?>
                    </select>
				</div>       
                <div class="col-md-1 right"> <label>เดือน</label> </div>
				<div class="col-md-2">  
					<select class="form-control" onchange="Common.jqgrid.onFilter()" name="MONTH">
                        <?php Utils::getOptionsMonth('', false); ?>
                    </select>
				</div>                                                                
...
jQuery('#btnUploadFile').click(function() {
	var url2 = "<?= Url::to(['ssbaerobic/uploadfile']); ?>";
...
jQuery("#rowed5").jqGrid({
	...
	width: jQuery('div.content').width()-20,
   	...
   	colModel:[
   		...
   		{name:'tmpyear',index:'YEAR',editable: false,editoptions:{size:"27",maxlength:"255"}},         
        {name:'tmpmonth',index:'MONTH',width:100,editable: false,editoptions:{size:"3",maxlength:"255"}},   
        {name:'agemale0',index:'AGE_MALE_0',width:35, align:'center', editable: true,editoptions:{size:"3",maxlength:"3"}},   
        {name:'agefemale0',index:'AGE_FEMALE_0',width:35, align:'center', editable: true,editoptions:{size:"3",maxlength:"3"}},   
...
var myEditOptions = {
        aftersavefunc: function (rowid, response, postdata) {
        	...
        	Common.jqgrid.uploadImage(this, "<?=Url::to(['ssbaerobic/uploadfile'])?>", 'input:file[name="fileToUpload"]', rowid, response, postdata);
        }
    };
...
jQuery("#rowed5").jqGrid('inlineNav',"#pagered", {edit:true, save:true, cancel:true,
	    ...
	    onafteraddfunc: function(){
        	...
        	// TODO: set tmp from filter
    		Common.jqgrid.setColumn(selector, 'tmpprovince', jQuery('[name="PROVINCE_CODE"] option:selected').text());
        	Common.jqgrid.setColumn(selector, 'tmpyear', jQuery('select[name="YEAR"]').val());
        	Common.jqgrid.setColumn(selector, 'tmpmonth', jQuery('[name="MONTH"] option:selected').text());

        	// TODO: set hidden value from filter
