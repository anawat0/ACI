...
<script type="text/javascript">
	var gridurl_1 = "<?= Url::to(['ssbmainproj/gridview']); ?>";
	var rtparams = Common.jqgrid.getDefaultPostData('<?= $rtparams; ?>', {sidx: 'SSB_MAIN_PROJ_ID'});

	function gotoSsbProj(t){
		var ssbMainProjId = jQuery(t).closest('tr[role="row"]').attr('id');
	    var url = '';

	    // TODO: check edit mode
	    if (!isNaN(ssbMainProjId)) {
	    	url += "<?= Url::to(['ssbproj/index']); ?>";
			url += '&ssb_main_proj_id=' + ssbMainProjId;
	...
	jQuery("#rowed5").jqGrid({
		...
		colNames:['ลำดับที่', 
					...
					'Actions'],
	   	colModel:[
	        {name:'id',index:'SSB_MAIN_PROJ_ID', width:80, align:'center', sorttype:"int", editable: true, editable: false},
			...
			{name:'budget_year', index:'BUDGET_YEAR', width:100, sortable:true, align:'center', sorttype:"int", editable: true, edittype:"select", editoptions:{style:"width:100%", value:<?= Json::encode(Utils::getArrYears(false)); ?>}},
			{name:'status',index:'STATUS', width:100, sortable:true, editable: true, align:'center', edittype:"checkbox", editoptions:{value:"ใช้งาน:ไม่ใช้งาน"}, formatter: Common.jqgrid.chkFormatter},
	   		{name:'sub', index:'sub', width:100, editable:false, sortable:false, align:'center', formatter: function(){return '<span style="margin:auto;cursor:pointer;" onclick="gotoSsbProj(this);" class="ui-icon ui-icon-circle-zoomout"></span>';}},
