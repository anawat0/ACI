...
// 	}
	...
	$imMainProjId = $imMainProj == null? '': $imMainProj->IM_MAIN_PROJ_ID;
?>
...
<script type="text/javascript">
	var gridurl_1 = "<?= Url::to(['improj/gridview', 'im_main_proj_id' => $imMainProjId]); ?>";
	var rtparams = Common.jqgrid.getDefaultPostData('<?= $rtparams; ?>', {sidx: 'IM_PROJ_ID'});

	function gotoImSubProj(t){
		var imProjId = jQuery(t).closest('tr[role="row"]').attr('id');
	    var url = '';

	    // TODO: check edit mode
	    if (!isNaN(imProjId)) {
	    	url = "<?= Url::to(['imsubproj/index']); ?>";
			url += '&im_proj_id=' + imProjId;
	...
	jQuery("#rowed5").jqGrid({
		...
		colNames:['ลำดับที่', 
					...
					'Actions'],
	   	colModel:[
	        {name:'id', index:'IM_PROJ_ID', width:80, align:'center', sorttype:"int", editable: true, editable: false},
			{name:'IM_MAIN_PROJ_ID', index:'IM_MAIN_PROJ_ID', hidden:true , editable:true, editrules:{edithidden:false}, editoptions:{defaultValue:'<?= $imMainProjId; ?>'}},
			...
			{name:'status',index:'STATUS', width:80, sortable:true, editable: true, align:'center', edittype:"checkbox", editoptions:{value:"ใช้งาน:ไม่ใช้งาน"}, formatter: Common.jqgrid.chkFormatter},
	   		{name:'sub', index:'sub', width:100, editable:false, sortable:false, align:'center', formatter: function(){return '<span style="margin:auto;cursor:pointer;" onclick="gotoImSubProj(this);" class="ui-icon ui-icon-circle-zoomout"></span>';}},
