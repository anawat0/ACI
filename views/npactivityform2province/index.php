...
<?= Html::beginForm('', 'post', ['id' => 'frmsearch', 'class' => 'form-horizontal form-filtering']); ?>    
	<div class="panel panel-primary">
	  <div class="panel-body">
	  		<div class="form-group">
				...
				<div class="col-md-3">
					<select class="form-control" onchange="Common.jqgrid.onFilter()"
						name="NP_ACTIVITY_FORM2_PROVINCE.YEAR">
				...
				<div class="col-md-3">
					<select class="form-control" onchange="Common.jqgrid.onFilter()"
						name="NP_ACTIVITY_FORM2_PROVINCE.MONTH">
                <?php Utils::getOptionsMonth(); ?>
            		</select>
				</div>
			</div>
	        <div class="form-group">
	        	<?= Html::label('จังหวัด', 'NP_ACTIVITY_FORM2_PROVINCE.PROVINCE_CODE', ['class' => 'col-md-2 control-label']); ?>
	        	<div class="col-md-3">
	        		<?php
			        	echo Utils::getDDLProvince_AutoComplete(null, null, 'NP_ACTIVITY_FORM2_PROVINCE.PROVINCE_CODE',
							'PROVINCE_CODE',
							'PROVINCE_NAME_TH',
							$province,
							$user_info,
        	...
        	<div class="form-group">
	        	<?= Html::label('รายงานการปฎิบัติงาน', 'NP_WORK_GROUP_ACTIVITY_ID', ['class' => 'col-md-2 control-label']); ?>
	        	<div class="col-md-8">
		            <?php
			        	echo Html::dropDownList('NP_ACTIVITY_FORM2_PROVINCE.NP_WORK_GROUP_ACTIVITY_ID',
			        								$act_id,
			        								BaseArrayHelper::map(NP_WORK_GROUP_ACTIVITY::find()->where($where_work)->all(), 
			        																'NP_WORK_GROUP_ACTIVITY_ID', 
			        																'ACTIVITY_SUBJECT'),
													[
														'prompt' => 'กรุณาเลือก',
														'id' => 'NP_WORK_GROUP_ACTIVITY_ID',
														'class' => 'form-control',
														'onchange' => 'Common.jqgrid.onFilter()',
													]);
					?>
...
<script type="text/javascript">
	var gridurl_1 = "<?=Url::to(['npactivityform2province/gridview']);?>";
	var rtparams = Common.jqgrid.getDefaultPostData('<?=$rtparams?>', {sidx: 'NP_ACTIVITY_FORM2_PROVINCE.NP_ACTIVITY_FORM2_PROVINCE_ID'});

	function getFilterParam() {
		var frmsearch = jQuery('#frmsearch');
		var url = '&act_id='+frmsearch.find('[name="NP_ACTIVITY_FORM2_PROVINCE.NP_WORK_GROUP_ACTIVITY_ID"]').val();

		url += '&work_id=<?=$work_id?>';
		url += '&year='+frmsearch.find('[name="NP_ACTIVITY_FORM2_PROVINCE.YEAR"]').val();
		url += '&month='+frmsearch.find('[name="NP_ACTIVITY_FORM2_PROVINCE.MONTH"]').val();
		url += '&province='+frmsearch.find('[name="NP_ACTIVITY_FORM2_PROVINCE.PROVINCE_CODE"]').val();
	...
	function gotoView2(t){
		var url='<?=Url::to(['npactivityform2province/add'])?>';
	...
	jQuery("#rowed5").jqGrid({
		...
		width: jQuery('div.content').width()-20,
	   	colNames:['ลำดับที่',
				   	...
				   	'แก้ไข'],
	   	colModel:[
	  	   		{name:'id',index:'NP_ACTIVITY_FORM2_PROVINCE_ID', align:'center', width:40, sorttype:"int", sortable:true, editable: false},
		   		...
		   		{name:'month',index:'MONTH', width:60, hidden:true},
		...
		loadComplete: function(data) {
			var frmsearch = jQuery('#frmsearch');
			var filterYear = frmsearch.find('[name="NP_ACTIVITY_FORM2_PROVINCE.YEAR"]').val();
			var filterMonth = frmsearch.find('[name="NP_ACTIVITY_FORM2_PROVINCE.MONTH"]').val();
