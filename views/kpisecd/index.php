...
var gridurl_1 = "<?=Url::to( ['kpisecd/gridview' , 'kpidimention'=>$kpidimention , 'kpifst'=>$kpifst] );?>";

var rtparams = Common.jqgrid.getDefaultPostData('', {sidx: 'SECD_NAME_TH'});

function gotoView1(t){
    var url='<?=Url::to(['kpithrd/'])?>';     
    	url += '&kpisecd=' + jQuery(t).closest('tr[role="row"]').attr('id');        
        url += '&kpidimention=<?=$kpidimention?>'  ;
        url += '&kpifst=<?=$kpifst?>' ;
        url += '&budgetyear=<?=$budgetyear?>'  ;
        url += '&dimentionnameth=<?=$dimentionnameth?>'  ;
        url += '&fstnameth=<?=$fstnameth?>'  ;
        url += '&secdnameth=' + jQuery(t).closest('tr[role="row"]').find('td')[2].innerHTML;
...
jQuery("#rowed5").jqGrid({
	...
	width: jQuery('div.content').width()-20,
   	colNames:['ลำดับที่',
                ...
                'Actions'],
   	colModel:[
   		{name:'id',index:'KPI_SECD_ID', width:60, align:'center', sorttype:"int", editable: false},
   		{name:'secdnameth',index:'SECD_NAME_TH',editable: true,editoptions:{maxlength:"255"}},   		
